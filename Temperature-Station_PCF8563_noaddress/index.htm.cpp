#include<pgmspace.h>
char index_htm[] PROGMEM = R"abcd(
<!DOCTYPE html><html lang="en"><head><meta http-equiv="cache-control" content="no-cache" /><title>Temperature Station</title><style type="text/css" media="screen">.contextMenu{
z-index:300;
position:absolute;
left:5px;
...
padding:5px;}
</style><script>function createFileUploader(element){
var sensors=document.createElement("button");
sensors.innerHTML='Sensors';
document.getElementById(element).appendChild(sensors);
var settings=document.createElement("button");
settings.innerHTML='Settings';
document.getElementById(element).appendChild(settings);
sensors.onclick=function(e){
...
if(isChecked){
expand.innerHTML="<span>Collapse</span>";
...
httpGet(leaf,path);
if(document.body.getElementsByClassName('contextMenu').length>0)document.body.removeChild(el);};}else{
expand.innerHTML="<span>Expand</span>";
expand.onclick=function(e){
...
list.appendChild(action);
if(isTextFile(path)){
action.innerHTML="<span>Open</span>";
action.onclick=function(e){
editor.loadUrl(path);
if(document.body.getElementsByClassName('contextMenu').length>0)document.body.removeChild(el);};}else if(isImageFile(path)){
...
editor.loadUrl(leaf.id);}else if(isImageFile(leaf.id)){
loadPreview(leaf.id);}};
...
var itemEl;
if(item.type==="file"){
itemEl=createTreeLeaf(path,item.name,item.size);}else{
...
alert("ERROR["+xmlHttp.status+"]:"+xmlHttp.responseText);}else{
if(path.lastIndexOf('/')<1){
path='/';
treeRoot.removeChild(treeRoot.childNodes[0]);
httpGet(treeRoot,"/");}else{
...
var temppath="/archiwum/"+year+month+"/"+day+".csv";
if(typeof file==="undefined")file=temppath;
