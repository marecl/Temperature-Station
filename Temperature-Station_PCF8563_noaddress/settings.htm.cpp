...
z.ch_ap.checked=false;
z.ch_wi.checked=false;
z=document.forms.time;
z.UN.addEventListener("change",function(){
var q=document.forms.time;
q.NS.disabled=q.UN.checked?false:true;});
z.NS.placeholder=resp.NS;
z.TZ.placeholder=resp.TZ;
z.UN.checked=resp.UN;}};
getTime();
return;}
function getTime(){
var xhr=new XMLHttpRequest();
...
if(xhr.status===200&&xhr.readyState===4){
console.log(xhr.responseText);
var z=document.forms.time.datetime;
var d=new Date(xhr.responseText);
z.valueAsDate=d;
console.log(z.valueAsDate);}}}
function uploadGen(){
var submitOK=true;
...
if(g.TZ.value<-12||g.TZ.value>12){
submitOK=false;
alert("Check timezone");}
if(g.TZ.value!=""){
postData.push("TZ="+g.TZ.value);}
if(g.UN.checked==true){
if(g.NS.value==""){
if(g.NS.placeholder==""){
returnOK=false;
alert("Check NTP address");}} else{
postData.push("NS="+g.NS.value);}
postData.push("UN=1");} else{
postData.push("UN=0");}
var epo=(new Date(g.datetime.value).getTime())/1000;
if(!isNaN(epo)){
